 ...
 *
 * Runtime : Node.js 18.x 이상 (AWS Lambda) — ES Module (.mjs)
...
function mapAsteroids(nearEarthObjects) {
  ...
  for (const date of Object.keys(nearEarthObjects)) {
    ...
    for (const neo of dailyList) {
      ...
      result.push({
        ...
        absoluteMagnitude:
          typeof neo.absolute_magnitude_h === 'number' ? neo.absolute_magnitude_h : null,
  ...
  // 지구와의 거리 오름차순 정렬 (가장 가까운 천체가 맨 위)
